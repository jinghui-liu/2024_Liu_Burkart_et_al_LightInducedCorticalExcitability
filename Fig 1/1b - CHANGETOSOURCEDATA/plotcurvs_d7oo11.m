...
rhotifpath = '/Volumes/One Touch/2022 Photoactivation - Analysis/Analysis_WildtypeSCW/d7oo1-14wt/d7oo11wt.tif';

% load(['bws_updated/bws_' filename '.mat'],'bws_x','bws_y','rad','thresh','smfac','sFac');

load('save_kymocurvextract_d7oo11wt_20220910 21_10/boundary.mat','bws');
...
load('save_kymocurvextract_d7oo11wt_20220910 21_10/curvs.mat','curvs');
...
for t = 105:1:105 % 1:lag_frame:size(imArr,3)
...
%     end
    ...
    imagesc(I.*mask);hold on;colormap(colorcet('L05'));caxis(rholim);
...
%             plot(col_cm+(col_bd(i_curv):col_bd(1)-col_cm).*dilate_fac,row_cm+(row_bd(i_curv):row_bd(1)-row_cm).*dilate_fac,'color',mycurvmap(colork,:),'linewidth',2);hold on;
    ...
    ylim([row_cm-160/umperpix row_cm+160/umperpix]);xlim([col_cm-160/umperpix col_cm+160/umperpix]);
