...
arcpoints = 80; Ninterpcurv = 5;% curv extract para for plotting polished figure;

curvs_t1_2 = [];

for k_file = 1:1:length(suffixlist)
    
    suffix = suffixlist{k_file};
    
%     spf = spflist{k_file};
    
    load(['curvs_updated/curvs_' filename suffix '.mat'],'curvs');
    
    curvs_t1_2 = [curvs_t1_2 curvs(1:1200,1:t_totallist{k_file})];
    
end

curvs_t1_2 = curvs_t1_2(:,1:379);

figure;
imagesc(curvs_t1_2);colorbar;caxis([0.003 0.006]);hold on;pbaspect([2,1,1]);

bws_x_t1_2 = cell(1,379);bws_y_t1_2 = cell(1,379);
...
curv_plotted = (curvs_interp_shiftVP(:,51:379)-curvs_interp_shiftVP0(:,51:379))./umperpix;
...
% imArr = readfromtiff('d2oo1_lighton_tseries2_spf12s_power15_ite3_globaltrack2on.czi - C=0',t_total-50,512,512);
% save(['imArr_' filename,'.mat'],'imArr','-v7.3');
% % load(['imArr_' filename,'.mat'],'imArr');

% imArr = readfromtiff('d2oo1_lighton_tseries2_spf12s_power15_ite3_globaltrack2on.czi - DIC',t_total-50,512,512);
% save(['imArr_' filename,'_dic.mat'],'imArr','-v7.3');
load(['imArr_' filename,'_dic.mat'],'imArr');
...
deltacurvs_interp = deltacurvs_interp(:,51:379);

dilate_fac = 1.2;

mincurv = -0.001; maxcurv = 0.001;

figure;

for t = 242:1:242 % 1:lag_frame:size(imArr,3) % 242,253,264
    
    disp(t);
    
    I = imArr(:,:,t);
    
    col_bd = bws_x{t+t_totallist{1}};row_bd = bws_y{t+t_totallist{1}};
    ...
    rhot_interp = rhos_interp(:,t+t_totallist{1})./umperpix; % transform from microns to pixels
...
dilated_col_bd_interp2 = col_cm+dilate_fac2.*rhot_interp.*cos(phis_interpcor_minuspitopi'); %%
dilated_col_bd_interp_sm2 = smooth(dilated_col_bd_interp2,20);

dilated_row_bd_interp2 = row_cm+dilate_fac2.*rhot_interp.*sin(phis_interpcor_minuspitopi'); %%
dilated_row_bd_interp_sm2 = smooth(dilated_row_bd_interp2,20);

[X,Y] = meshgrid(1:1:size_col,1:1:size_row);
mask = inpolygon(X,Y,dilated_col_bd_interp_sm2,dilated_row_bd_interp_sm2); % get rid of fluorescence signals outside the circle

I(~mask) = 96;

    %%
    
    imagesc(I);hold on;colormap(colorcet('L01'));% caxis(geflim);
    clim([40 135]);
...
%             plot(col_cm+(col_bd(i_curv):col_bd(1)-col_cm).*dilate_fac,row_cm+(row_bd(i_curv):row_bd(1)-row_cm).*dilate_fac,'color',mycurvmap(colork,:),'linewidth',2);hold on;
    ...
    title(['frame ' num2str(t) ', ' num2str(taxis(t+t_totallist{1})) ' min'],'fontsize',14);
...
fig_current = gcf; fig_current.Renderer = 'painters';
print(fig_current,[savedir '/t' num2str(t) '_deltacurv_' '_cetD09_highthres_DIC'],'-dpdf');
